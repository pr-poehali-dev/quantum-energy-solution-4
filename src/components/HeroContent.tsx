...
export default function HeroContent() {
  ...
  return (
    <>
      <main className="absolute bottom-4 left-4 sm:bottom-8 sm:left-8 z-20 max-w-xs sm:max-w-lg">
        <div className="text-left">
          <p className="text-xs sm:text-sm font-light text-white/70 mb-4 leading-relaxed line-clamp-4 sm:line-clamp-none">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
            <button
              ...
              className="px-5 sm:px-8 py-2.5 sm:py-3 rounded-full bg-transparent border border-white/30 text-white font-normal text-xs transition-all duration-200 hover:bg-white/10 hover:border-white/50 cursor-pointer"
            >
              Наши услуги
            </button>
            <button
              onClick={() => setModalOpen(true)}
              className="px-5 sm:px-8 py-2.5 sm:py-3 rounded-full bg-white text-black font-normal text-xs transition-all duration-200 hover:bg-white/90 cursor-pointer"
            >
              Связаться с нами
            </button>
          </div>
